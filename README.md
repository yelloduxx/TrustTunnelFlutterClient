...
After generating the configuration on the server, open the **TrustTunnel Flutter Client** and navigate to the **Servers** section.

You can import settings automatically:
- tap **Import config** and pick a `.toml` file, or
- open a `tt://` link generated by the admin panel.

Manual setup is still available through **Add Server / Edit Server**.
